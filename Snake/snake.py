...
sql = '''
    SELECT * FROM users_snake WHERE username = %s;
'''
...
if data == None:
    sql = '''
        INSERT INTO users_snake VALUES(%s, 0, 0, 0);
    '''
    current.execute(sql, [username])
    config.commit()
pygame.init()
...
while not finished:
    ...
    if score > highscore:
        highscore = score
        sql = '''
        UPDATE users_snake SET score = %s, highscore = %s, level = %s WHERE username = %s;
        '''
...
if score > highscore:
    ...
    level = 0
    sql = '''
        UPDATE users_snake SET score = %s, level = %s WHERE username = %s;
    '''
    current.execute(sql,[score, level, username])
sql = '''
    SELECT * FROM users_snake;
'''
current.execute(sql, [username])
